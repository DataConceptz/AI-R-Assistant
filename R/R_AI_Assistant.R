...
# Prereqs: install.packages(c("shiny","shinyAce","httr","jsonlite","digest","memoise","keyring","shinyBS","rintrojs","ggplot2","plotly","viridis","patchwork","scales","corrplot"))
...
if (!isNamespaceLoaded("aiRAssistant")) {
  ...
  # Optional: Load if available for advanced charts
...
ui <- fluidPage(
  ...
  tags$head(tags$style(HTML("
    ...
    * { box-sizing: border-box; }
    ...
    .console-section { background: #0a0a14; border-top: 2px solid #00ffaa; display: flex; flex-direction: column; flex: 0 0 auto; min-height: 36px; max-height: 160px; }
    .console-section.collapsed { flex: 0 0 36px; min-height: 36px; max-height: 36px; }
          ...
          div(id = "cloud_config", class = "cloud-config", style = "display:none;",
            ...
            conditionalPanel("input.cloud_provider == 'openai'",
              ...
              div(class = "settings-row",
                ...
                div(class = "settings-input",
                  textInput("api_key_openai", NULL, placeholder = "OPENAI_API_KEY or keyring: openai_api_key", width = "100%")
                )
              )
            ),
              ...
              div(class = "settings-row",
                ...
                div(class = "settings-input",
                  selectInput("anthropic_model", NULL,
                              choices = list("Sonnet 3.5 (Latest)" = "claude-3-5-sonnet-20241022",
                                          "Haiku 3.5 (Fast)" = "claude-3-5-haiku-20241022"),
                              selected = "claude-3-5-haiku-20241022", width = "200px")
                )
              ),
              div(class = "settings-row",
                div(class = "settings-label", "API Key:"),
                div(class = "settings-input",
                  textInput("api_key_anthropic", NULL, placeholder = "ANTHROPIC_API_KEY or keyring: anthropic_api_key", width = "100%")
                )
              )
            ),
              ...
              div(class = "settings-row",
                ...
                div(class = "settings-input",
                  selectInput("gemini_model", NULL,
                              choices = list("gemini-1.5-flash" = "gemini-1.5-flash", "gemini-1.5-pro" = "gemini-1.5-pro"),
                              selected = "gemini-1.5-flash", width = "200px")
                )
              ),
              div(class = "settings-row",
                div(class = "settings-label", "API Key:"),
                div(class = "settings-input",
                  textInput("api_key_gemini", NULL, placeholder = "GEMINI_API_KEY or keyring: gemini_api_key", width = "100%")
                )
              )
            )
          ),
...
devtools::install_local('C:/Users/GADSPAA/Documents/R-Assistant', upgrade = 'never')\n
  ...
  theme(legend.position = "none",
              ...
              div(class = "plot-pane",
                div(class = "plot-toolbar",
                  ...
                  tags$div(class = "plot-toolbar-right",
                    downloadButton("save_plot", "Save Plot", class = "quick-btn"),
        ...
        div(class = "console-section",
          div(class = "console-header",
            ...
            actionButton("toggle_console", "Collapse", class = "quick-btn", onclick = "toggleConsole();")
          ),
  ...
  tags$script(HTML("
    document.addEventListener('click', function(e) {
      ...
      while (el && el !== document) {
        ...
        if (el.id === 'toggle_console') { _doToggleConsole(); return; }
        el = el.parentElement;
      }
    });
    function _doEditorTab(tab) {
      tab = (tab === 'plot') ? 'plot' : 'editor';
    ...
    function _doChatTab(tab) {
      var sc = tab === 'code';
      var ct = document.getElementById('chat_tab_chat');
      var cd = document.getElementById('chat_tab_code');
      var cp = document.getElementById('chat_pane_chat');
      var dp = document.getElementById('chat_pane_code');
      if (ct) ct.classList.toggle('active', !sc);
      if (cd) cd.classList.toggle('active', sc);
      if (cp) cp.classList.toggle('active', !sc);
      if (dp) dp.classList.toggle('active', sc);
    }
    function _doToggleConsole() {
      var c = document.querySelector('.console-section');
      if (!c) return;
      c.classList.toggle('collapsed');
      var btn = document.getElementById('toggle_console');
      if (btn) btn.innerText = c.classList.contains('collapsed') ? 'Expand' : 'Collapse';
    }
  ...
  tags$script(HTML("
          ...
          if (mutation.addedNodes) {
            mutation.addedNodes.forEach(function(node) {
              if (node.classList && node.classList.contains('modal')) {
                // Modal appeared, trigger resize after render
                setTimeout(function() {
                  try { Shiny.setInputValue('popout_resize', Date.now(), {priority: 'event'}); } catch (e) {}
                  try {
                    var pl = document.getElementById('plotly_display_popout');
    ...
    function setEditorTabClient(tab) {
      ...
      if (tab === 'editor') {
        ...
        setTimeout(function() {
          // Ensure plotly reflows after being shown (fixes clipped/blank charts).
          try {
            var pl = document.getElementById('plotly_display');
    ...
    function toggleConsole() {
      var console = document.querySelector('.console-section');
      if (!console) return;
      console.classList.toggle('collapsed');
      var btn = document.querySelector('#toggle_console');
      if (btn) btn.innerText = console.classList.contains('collapsed') ? 'Expand' : 'Collapse';
    }
    ...
    function switchChatTab(tab) {
      var chatTab = document.getElementById('chat_tab_chat');
      var codeTab = document.getElementById('chat_tab_code');
      var chatPane = document.getElementById('chat_pane_chat');
      var codePane = document.getElementById('chat_pane_code');
      if (!chatTab || !codeTab || !chatPane || !codePane) return;
      var showCode = tab === 'code';
      chatTab.classList.toggle('active', !showCode);
      codeTab.classList.toggle('active', showCode);
      chatPane.classList.toggle('active', !showCode);
      codePane.classList.toggle('active', showCode);
...
server <- function(input, output, session) {
  ...
  get_cloud_api_key <- function(provider) {
    ...
    if (identical(provider, "openai")) {
      if (nzchar(input$api_key_openai)) return(input$api_key_openai)
      key <- Sys.getenv("OPENAI_API_KEY", "")
      if (nzchar(key)) return(key)
      kr <- tryCatch({ keyring::key_get("openai_api_key") }, error = function(e) "")
    ...
    if (identical(provider, "anthropic")) {
      if (nzchar(input$api_key_anthropic)) return(input$api_key_anthropic)
      key <- Sys.getenv("ANTHROPIC_API_KEY", "")
      if (nzchar(key)) return(key)
      kr <- tryCatch({ keyring::key_get("anthropic_api_key") }, error = function(e) "")
      if (nzchar(kr)) return(kr)
      return("")
    }

    if (identical(provider, "gemini")) {
      if (nzchar(input$api_key_gemini)) return(input$api_key_gemini)
      key <- Sys.getenv("GEMINI_API_KEY", "")
      if (nzchar(key)) return(key)
      kr <- tryCatch({ keyring::key_get("gemini_api_key") }, error = function(e) "")
      if (nzchar(kr)) return(kr)
      return("")
    }
...
print(analyze_variable(mtcars, \"mpg\", \"cyl\"))"
  ...
  format_code_blocks <- function(text) {
    ...
    if (length(blocks) == 0 || blocks[1] == -1) return(text)
    for (block in blocks) {
      lang <- sub("^```([a-zA-Z0-9]*)\\n[\\s\\S]*\\n```$", "\\1", block, perl = TRUE)
      code <- sub("^```[a-zA-Z0-9]*\\n([\\s\\S]*?)\\n```$", "\\1", block, perl = TRUE)
  ...
  extract_first_code_block <- function(text) {
    if (!nzchar(text)) return("")
    # Robust pattern for any markdown code block, handling various line endings and optional language tags.
    # Newlines before/after code are optional to handle varied AI response styles.
    pattern <- "```(?:[a-zA-Z0-9]*)\\s*[\\r\\n]*([\\s\\S]*?)[\\r\\n]*```"
    m <- regexec(pattern, text, perl = TRUE)
    res <- regmatches(text, m)
    if (length(res) > 0 && length(res[[1]]) >= 2) {
      return(trimws(res[[1]][2]))
    }
    # Fallback: if text looks like pure code without blocks
  ...
  call_openai_api <- function(user_message, code_context, model, api_key, max_tokens = 3000) {
    prompt <- paste0(
  ...
  call_anthropic_api <- function(user_message, code_context, model, api_key, max_tokens = 3000) {
    prompt <- paste0(
      "You are an expert R programming and data visualization assistant. Help with R coding, ",
      "debugging, explanations, and especially creating advanced charts with ggplot2, plotly, and other packages.\n\n",
      "Current R code context:\n```r\n", code_context, "\n```\n\n",
  ...
  call_gemini_api <- function(user_message, code_context, model, api_key) {
    prompt <- paste0(
      "You are an expert R programming and data visualization assistant. Help with R coding, ",
      "debugging, explanations, and especially creating advanced charts with ggplot2, plotly, and other packages.\n\n",
      "Current R code context:\n```r\n", code_context, "\n```\n\n",
  ...
  call_ollama_api <- function(user_message, code_context, model, base_url, max_tokens = 3000) {
    base_url <- normalize_ollama_url(base_url)
    if (!nzchar(base_url)) {
      return(paste0(translate("ollama_error"), " Missing Ollama URL."))
    }

    # Truncate very large code contexts to keep prompts fast
    max_ctx <- 4000
    if (nchar(code_context) > max_ctx) {
      code_context <- paste0(substr(code_context, 1, max_ctx), "\n# ... (truncated)")
    }

    system_msg <- paste0(
      "You are an expert R programming and data visualization assistant. ",
      "Provide clear, concise responses with complete, working R code. ",
      "For charts, use ggplot2 with proper theming. Keep explanations brief."
    )

    user_content <- if (nzchar(code_context)) {
      paste0("Current code:\n```r\n", code_context, "\n```\n\n", user_message)
    } else {
      user_message
    }
  ...
  cached_openai_call <- memoise::memoise(call_openai_api)
  cached_anthropic_call <- memoise::memoise(call_anthropic_api)
  cached_gemini_call <- memoise::memoise(call_gemini_api)
  ...
  stream_openai_chat <- function(prompt, model, api_key, on_delta, max_tokens = 3000) {
  ...
  stream_anthropic_messages <- function(prompt, model, api_key, on_delta, max_tokens = 3000) {
  ...
  stream_ollama_chat <- function(prompt, model, base_url, on_delta, code_context = "", max_tokens = 3000) {
    base_url <- normalize_ollama_url(base_url)
    ...
    curl::handle_setopt(h, post = TRUE, postfields = body_json, timeout = 60)
    buf <- ""
    ...
    tryCatch({
      ...
      if (!nzchar(acc)) {
        if (grepl("Timeout|timed out", msg_text, ignore.case = TRUE)) {
          acc <<- "Streaming timed out. Try a smaller model or check Ollama."
        } else {
  ...
  send_message <- function(user_message, mode = "chat") {
    ...
    ai_resp <- if (model_type() == "cloud") {
      ...
      if (can_stream()) {
        tryCatch({
          if (identical(provider, "openai")) {
            stream_openai_chat(prompt, cloud_model, api_key, function(delta, acc) { update_assistant(acc); maybe_flush() }, max_tokens = input$max_tokens_setting)
          } else if (identical(provider, "anthropic")) {
            stream_anthropic_messages(prompt, cloud_model, api_key, function(delta, acc) { update_assistant(acc); maybe_flush() }, max_tokens = input$max_tokens_setting)
          } else {
            # Gemini fallback to cached call
            cached_gemini_call(user_message, code_context, cloud_model, api_key, input$max_tokens_setting)
          }
        }, error = function(e) {
          msg_text <- error_message(e)
        ...
        tryCatch(
          switch(provider,
            openai = cached_openai_call(user_message, code_context, cloud_model, api_key, input$max_tokens_setting),
            anthropic = cached_anthropic_call(user_message, code_context, cloud_model, api_key, input$max_tokens_setting),
            gemini = cached_gemini_call(user_message, code_context, cloud_model, api_key, input$max_tokens_setting),
            cached_openai_call(user_message, code_context, cloud_model, api_key, input$max_tokens_setting)
          ),
      ...
      if (can_stream()) {
        tryCatch(
          ...
          }, code_context = code_context, max_tokens = input$max_tokens_setting),
          error = function(e) {
            msg_text <- error_message(e)
            if (grepl("__STOPPED_BY_USER__", msg_text, fixed = TRUE)) return(NULL)
        ...
        tryCatch(
          call_ollama_api(user_message, code_context, input$ollama_model, input$ollama_url, input$max_tokens_setting),
          error = function(e) {
            msg_text <- error_message(e)
            if (grepl("__STOPPED_BY_USER__", msg_text, fixed = TRUE)) return(NULL)
            paste(translate("ollama_error"), msg_text)
    ...
    if (nzchar(ai_code)) {
      ai_code <- format_code_text(ai_code)
  ...
  insert_ai_code <- function() {
    ...
    if (nzchar(code_block %||% "")) {
      ...
      # Run the code through the full execution pipeline (same as Run button)
      run_code(code_block)
  ...
  run_code <- function(code) {
    ...
    text_out <- tryCatch({
      ...
      out <- capture.output({
        ...
        if (is_base_plot && !is_ggplot && !is_plotly) {
          ...
          # ggplot / plotly: the last expression value IS the plot object
          result <- eval(parsed_code, envir = new.env(parent = globalenv()))
        }
    ...
    if (has_plot && !is.null(current_plot())) {
      switch_editor_tab("plot")
    } else {
      switch_editor_tab("editor")
    }
  ...
  update_plot_only <- function(code, quiet = FALSE) {
    ...
    tryCatch({
      ...
      if (!is.null(result) &&
          (inherits(result, "ggplot") || inherits(result, "plotly") || inherits(result, "recordedplot"))) {
        # Pre-validate ggplot objects: catch scale mismatches BEFORE renderPlot
        if (inherits(result, "ggplot")) {
  ...
  apply_theme <- function(theme_id) {
    ...
    # Remove any leftover theme_pub definition
    code <- sub("(?s)theme_pub\\s*<-\\s*function\\([^)]*\\)\\s*\\{.*?\\}\\s*\\n*", "", code, perl = TRUE)

    theme_pat <- "(ggplot2::)?theme_(minimal|classic|bw|dark|light|void|grey|gray|linedraw|pub)"
    if (!grepl(theme_pat, code)) {
      showNotification("No ggplot2 theme found in code.", type = "warning")
      return()
    }
    code <- gsub(theme_pat, theme_id, code)
    ...
    content = function(file) {
      ...
      if (inherits(result, "ggplot")) {
        ggplot2::ggsave(file, plot = result, device = fmt,
                        width = 8, height = 5, dpi = if (fmt == "svg") 72 else 150)
      } else if (inherits(result, "plotly") && requireNamespace("htmlwidgets", quietly = TRUE)) {
        # Save interactive plotly as standalone HTML
        ...
        if (fmt == "svg") {
          ...
          png(file, width = 8 * 150, height = 5 * 150, res = 150)
        }
        replayPlot(result)
        dev.off()
      } else {
